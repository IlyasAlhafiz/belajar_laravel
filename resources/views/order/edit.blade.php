...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ isset($order) ? 'Edit order' : 'Tambah order' }}</div>

                <div class="card-body">
                    <form method="POST" action="{{ isset($order) ? route('order.update', $order->id) : route('order.store') }}" enctype="multipart/form-data">
                        @csrf
                        @if(isset($order))
                            @method('PUT')
                        @endif
